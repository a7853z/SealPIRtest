...
int main(int argc, char *argv[]) {
  ...
  uint64_t size_per_item = 23; // in bytes
  uint32_t N = 4096;
  ...
  string g_string = g_stream.str();
  cout<<"galois key length:"<<g_string.length()<<endl;
  ...
  server.write_db2disk("processed_dbs/db_200.bin");
   */
  auto time_pre_s = high_resolution_clock::now();
  server.read_db_from_disk(200);
  ...
  cout<<"reply vector length:"<<reply.size()<<endl;
  ...
  reply = client.deserialize_reply(reply.size(), server_stream);
