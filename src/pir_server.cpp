...
void PIRServer::set_galois_key(uint32_t client_id, string &g_string) {
    GaloisKeys server_galois;
    stringstream g_stream;
    g_stream<<g_string;
    server_galois.load(*context_, g_stream);
    galoisKeys_[client_id] = server_galois;
    g_string.clear();
    g_stream.clear();
    g_stream.str("");
}
...
// 将db_写到文件中
void PIRServer::write_db2disk(char * db_file) {
    ofstream os(db_file, ios::binary | ios::out);
    size_t size1 = db_->size();
    ...
    for (size_t i = 0; i < size1; i++) {
        pt = (*db_)[i];
        uint32_t data_size = pt.save(stream);
        os.write(reinterpret_cast<char*>(&data_size), streamsize(sizeof(uint32_t)));
        string pt_string = stream.str();
        const char * pt_temp = pt_string.c_str();
        os.write(pt_temp, data_size);
        stream.clear();
        stream.str("");
        pt_string.clear();
    }
...
void PIRServer::read_db_from_disk(uint32_t id_mod) {
    clear_db();
    uint64_t prod = 1;
    for (uint32_t i = 0; i < pir_params_.nvec.size(); i++) {
        prod *= pir_params_.nvec[i];
    ...
    auto result = DbCenter::read_db_from_disk(id_mod, *context_, prod);

    set_database(move(result));
}


void PIRServer::read_db_from_cache(uint32_t id_mod) {
    clear_db();
    uint64_t prod = 1;
    for (uint32_t i = 0; i < pir_params_.nvec.size(); i++) {
        prod *= pir_params_.nvec[i];
    }
    auto result = DbCenter::get_instance().get_db(id_mod, *context_, prod);
    set_database(move(result));
}

// 清空db_
void PIRServer::clear_db() {
    db_.reset();
}

void PIRServer::updata_pir_params(const PirParams &pir_parms) {
