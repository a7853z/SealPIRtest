...
public:
  ...
  void set_galois_key(std::uint32_t client_id, string & g_string);
  ...
  void updata_pir_params(const PirParams &pir_parms);
  void write_db2disk(char * db_file);
  void read_db_from_disk(uint32_t id_mod);
  void read_db_from_cache(uint32_t id_mod);
  void clear_db();
